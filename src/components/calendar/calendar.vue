  ...
  <div v-if="type!=='time'">
    ...
    <table cellpadding="5">
      ...
      <tr v-for="(day,dayIndex) in days" :key="day">
        <td v-for="(child,childIndex) in day" :key="child" :class="{'selected':child.selected,'disabled':child.disabled}" @click="select(dayIndex,childIndex,$event)" @touchstart="select(k1,k2,$event)">
          <span>{{child.day}}</span>
